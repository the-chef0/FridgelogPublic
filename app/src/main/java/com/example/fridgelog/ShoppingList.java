...
class SNode implements Parcelable
{
    ...
    protected SNode(Parcel in)
    {
        item = in.readParcelable(FridgeItem.class.getClassLoader());
        prev = in.readParcelable(SNode.class.getClassLoader());
        next = in.readParcelable(SNode.class.getClassLoader());
    }
    public static final Creator<SNode> CREATOR = new Creator<SNode>()
    {
        @Override
        public SNode createFromParcel(Parcel in) {
            return new SNode(in);
        }
        @Override
        public SNode[] newArray(int size) {
...
public class ShoppingList implements Parcelable
{
    ...
    {
        head = in.readParcelable(SNode.class.getClassLoader());
        tail = in.readParcelable(SNode.class.getClassLoader());
        size = in.readInt();
    }
    @Override
    public void writeToParcel(Parcel dest, int flags)
    ...
    public void delete(int index)
    {
        ...
        if (index == 0)
        {
            ...
            //this.head.prev = null;  //commenting this might cause problems later on
    ...
    public String[][] toArrays()
    {
        ...
        //We're making a 2D array so that we can store all 3 variables for each item
        String[][] arrays = new String[2][this.size()];
        ...
        while (curr != null)
        {
            ...
            arrays[1][i] = Byte.toString(curr.item.isTicked());
            i++;
            curr = curr.next;
        }
        return arrays;
    }
    public ArrayList<ShoppingListItem> toArrayList()
    {
        ...
        while(curr != null)
        {
            shoppingListItems.add(curr.item);
            curr = curr.next;
        }
